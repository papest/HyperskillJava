...
class TransactionExample {
    ...
    public static long calcSumOfCanceledTransOnNonEmptyAccounts(List<Account> accounts) {
        return accounts.stream().filter(account -> account.balance != 0L)
                .mapToLong(acc -> acc.transactions.stream()
                        .filter(tr -> tr.state.equals(State.CANCELED)).map(Transaction::getSum)
                        .reduce(0L, Long::sum)).sum();
    }
