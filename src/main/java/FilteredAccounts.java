...
class FilteredAccounts {

    public static void printFilteredAccounts(List<Account> accounts) {
        List<Account> nonEmptyAccounts = new ArrayList<>();
        List<Account> accountsWithTooMuchMoney = new ArrayList<>();
        if (accounts != null) {
            nonEmptyAccounts = filter(accounts, a -> a.getBalance() > 0L);
            accountsWithTooMuchMoney = filter(accounts, a -> !a.isLocked() && a.getBalance() >= 100000000L);
        }
